...
export const readStoredTrend = () => localStorage.getItem('rabtpoint_trend_country') || '';

export const storeTrendCountry = (country = '') => {
  if (country) localStorage.setItem('rabtpoint_trend_country', country);
};

export const initialTrendSearch = (user) => {
  const stored = readStoredTrend();
  const country = stored || user?.location?.country || 'United Kingdom';
  return trendLabelForCountry(country);
};
